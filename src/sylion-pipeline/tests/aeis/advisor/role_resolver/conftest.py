"""Role resolver fixtures for deterministic local advisor tests."""
...
def _role_resolver_local_runtime(monkeypatch, tmp_path):
    from sylion.aeis.advisor.preferences import _db as preferences_db
    from sylion.aeis.advisor.preferences.service import reset_preferences_service
    from sylion.aeis.advisor.role_resolver.service import reset_role_resolver_service

    reset_preferences_service()
    reset_role_resolver_service()
    if preferences_db._sqlite_conn is not None:
        preferences_db._sqlite_conn.close()
    preferences_db._sqlite_conn = None
    monkeypatch.delenv("SYLION_ADVISOR_FORCE_PG", raising=False)
    monkeypatch.setenv("SYLION_DB_MODE", "sqlite")
    monkeypatch.setattr(
        preferences_db,
        "_sqlite_db_path",
        lambda: str(tmp_path / "advisor_preferences.db"),
    )
